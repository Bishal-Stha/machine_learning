...
age = st.number_input("Enter age:", min_value=10, max_value=100, value=18)
gender = st.radio("Gender:", ['Male', 'Female'])
study_hours_per_day = st.number_input("Study hours per day:", min_value=0.0, step=0.1)
social_media_hours = st.number_input("Social media hours per day:", min_value=0.0, step=0.1)
netflix_hours = st.number_input("Netflix hours per day:", min_value=0.0, step=0.1)
part_time_job = st.radio("Do you have a part-time job?", ['Yes', 'No'])
attendance_percentage = st.slider("Attendance percentage:", 0, 100, 85)
sleep_hours = st.number_input("Sleep hours per day:", min_value=0.0, step=0.1)
diet_quality = st.selectbox("Diet quality:", ['Poor', 'Fair', 'Good'])
exercise_frequency = st.slider("Exercise days per week:", 0, 7, 2)
parental_education_level = st.selectbox("Parent's Education Level:", ['High School', 'Bachelor', 'Master'])
internet_quality = st.selectbox("Internet Quality:", ['Poor', 'Average', 'Good'])
mental_health_rating = st.slider("Mental Health Rating (1-10):", 1, 10, 5)
extracurricular_participation = st.radio("Participate in extracurriculars?", ['Yes', 'No'])
...
parent_edu_map = {'High School': 1, 'Bachelor': 2, 'Master': 3}
